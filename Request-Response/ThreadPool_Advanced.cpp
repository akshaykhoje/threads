...
void processRequests(int req_id) {
	{
        std::lock_guard<std::mutex> lock(log_mtx);
        std::cout << "[Request " << req_id << "] is being processed by a thread..." << std::endl;
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
	return;
}
...
int main() {
    ...
    for(int i=0; i<1000; i++) {
    	pool.enqueue(std::bind(processRequests,i));
    }

	std::cout << "--- Processed all incoming requests --- " << std::endl;
